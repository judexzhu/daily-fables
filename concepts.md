...
{% assign order = "distributed-systems,kubernetes,networking,linux,storage,security,aws,performance,memory,runtime,observability,operations,scheduling,databases,virtualization,etcd" | split: "," %}
{% assign labels = "Distributed systems,Kubernetes,Networking,Linux,Storage,Security,AWS,Performance,Memory,Runtime,Observability,Operations,Scheduling,Databases,Virtualization,etcd" | split: "," %}
{% assign labels_zh = "分布式系统,Kubernetes,网络,Linux,存储,安全,AWS,性能,内存,运行时,可观测性,运维,调度,数据库,虚拟化,etcd" | split: "," %}
